...
use Carbon\Carbon;
$title = "Tambah Setoran Hafalan - " . htmlspecialchars($santri->nama);
$activeMenu = "santri";
ob_start();
?>
<div class="card-custom p-4 mx-auto" style="max-width: 600px;">
    <h3 class="mb-4">Setoran Hafalan Baru</h3>
    ...
    <form method="POST" action="index.php?action=ustadz/setoran/store_hafalan&id=<?= $santri->id ?>">
        ...
        <div class="row">
            <div class="col-md-6 mb-3"><label>Ayat Mulai</label><input type="number" name="ayat_mulai" class="form-control" required></div>
            <div class="col-md-6 mb-3"><label>Ayat Selesai</label><input type="number" name="ayat_selesai" class="form-control" required></div>
        </div>
        ...
        <a href="index.php?action=ustadz/santri/show&id=<?= $santri->id ?>" class="btn btn-secondary w-100 mt-2">Batal</a>
    </form>
</div>
<?php
$content = ob_get_clean();
// Perbaikan path: dari views/ustadz/setoran/ ke views/layouts/main.php (naik 2 tingkat)
